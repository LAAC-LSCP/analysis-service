...
class VTC(ModelPlugin):
    ...
    def postprocess(
        ...
        if not all_rttm:
            self._logger.warning(f"Expected output file {all_rttm!s} not found")
            return
    ...
    def _run_subprocess(self, bash_script: str, output_dir: Path, file: Path) -> None:
        ...
        if result.returncode == 0:
            self._logger.info(f"Successfully ran VTC on '{file!s}'")
        else:
            self._logger.error(f"Error running VTC on '{file!s}: {result.stderr}")
